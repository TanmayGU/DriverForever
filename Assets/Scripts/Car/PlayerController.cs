...
public class PlayerController : MonoBehaviour
{
    ...
    private void AnalyzeAudio()
    {
        ...
        float highVolumeThreshold = 0.2f;

        // Adjust player shape based on volume levels
        if (volume < silenceThreshold || (volume>lowVolumeThreshold && volume<highVolumeThreshold))
        {
            targetScale = originalScale;
            Debug.Log("Silence detected: Keeping original shape.");
        }
        else if (volume < lowVolumeThreshold && volume>silenceThreshold)
        {
